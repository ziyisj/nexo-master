...
bool Memory::Init(std::string process_name, bool memMap, bool debug)
{
	if (!DMA_INITIALIZED)
	{
		LOG("inizializing...\n");
	reinit:
		LPCSTR args[] = { const_cast<LPCSTR>(""), const_cast<LPCSTR>("-device"), const_cast<LPCSTR>("fpga://algo=0"), const_cast<LPCSTR>(""), const_cast<LPCSTR>(""), const_cast<LPCSTR>(""), const_cast<LPCSTR>(""), const_cast<LPCSTR>(""), const_cast<LPCSTR>("") };
		//LPCSTR args[] = { const_cast<LPCSTR>(""), const_cast<LPCSTR>("-device"), const_cast<LPCSTR>("pmem"), const_cast<LPCSTR>(""), const_cast<LPCSTR>(""), const_cast<LPCSTR>(""), const_cast<LPCSTR>(""), const_cast<LPCSTR>(""), const_cast<LPCSTR>("") };
		DWORD argc = 3;
		if (debug)
		{
			args[argc++] = const_cast<LPCSTR>("-v");
		...
		args[argc++] = const_cast<LPCSTR>("-forensic");
		args[argc++] = const_cast<LPCSTR>("1");
...
bool Memory::FixCr3()
{
	...
	std::vector<UINT64> possible_dtbs = { };
...
UINT64 Memory::FindSignature(const char* signature, UINT64 range_start, UINT64 range_end, int PID)
{
	...
	for (UINT64 i = range_start; i < range_end; i++)
	{
...
bool Memory::ReadUseCache(uintptr_t address, void* buffer, size_t size) const
{
	DWORD read_size = 0;
	if (!VMMDLL_MemReadEx(this->vHandle, current_process.PID, address, static_cast<PBYTE>(buffer), size, &read_size, 0x0))
	{
		LOG("[!] Failed to read Memory at 0x%p\n", address);
		return false;
	}
...
VMMDLL_SCATTER_HANDLE Memory::CreateScatterHandleUseCache() const
{
	const VMMDLL_SCATTER_HANDLE ScatterHandle = VMMDLL_Scatter_Initialize(this->vHandle, current_process.PID, 0x0);
	if (!ScatterHandle)
		LOG("[!] Failed to create scatter handle\n");
	return ScatterHandle;
}
...
void Memory::AddScatterReadRequest(VMMDLL_SCATTER_HANDLE handle, UINT64 address, void* buffer, size_t size)
{
...
void Memory::AddScatterWriteRequest(VMMDLL_SCATTER_HANDLE handle, UINT64 address, void* buffer, size_t size)
{
...
void Memory::ExecuteReadScatterUseCache(VMMDLL_SCATTER_HANDLE handle, int pid)
{
	...
	//Clear after using it
	if (!VMMDLL_Scatter_Clear(handle, pid, 0x0))
	{
		LOG("[-] Failed to clear Scatter\n");
	}
}
