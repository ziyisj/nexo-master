...
public:
	...
	*/
	 ...
	 */
	UINT64 FindSignature(const char* signature, UINT64 range_start, UINT64 range_end, int PID = 0);
	...
	*/
	...
	bool ReadUseCache(uintptr_t address, void* buffer, size_t size) const;
	...
	*/
	...
	T Read(void* address)
	{
		...
		Read(reinterpret_cast<UINT64>(address), reinterpret_cast<void*>(&buffer), sizeof(T));
	...
	T ReadUseCache(void* address)
	{
		T buffer{ };
		memset(&buffer, 0, sizeof(T));
		ReadUseCache(reinterpret_cast<UINT64>(address), reinterpret_cast<void*>(&buffer), sizeof(T));

		return buffer;
	}

	template <typename T>
	T ReadUseCache(UINT64 address)
	{
		return ReadUseCache<T>(reinterpret_cast<void*>(address));
	}
	...
	*/
	...
	T Read(void* address, int pid)
	{
		T buffer { };
		memset(&buffer, 0, sizeof(T));
		Read(reinterpret_cast<UINT64>(address), reinterpret_cast<void*>(&buffer), sizeof(T), pid);

		return buffer;
	}

	template <typename T>
	T Read(UINT64 address, int pid)
	{
	...
	*/
	UINT64 ReadChain(UINT64 base, const std::vector<UINT64>& offsets)
	{
		UINT64 result = Read<UINT64>(base + offsets.at(0));
		for (int i = 1; i < offsets.size(); i++) result = Read<UINT64>(result + offsets.at(i));
		return result;
	}

	template <typename T>
	T ReadChain(UINT64 base, const std::vector<UINT64>& offsets)
	{
	...
	VMMDLL_SCATTER_HANDLE CreateScatterHandleUseCache() const;
	 ...
	 */
	void AddScatterReadRequest(VMMDLL_SCATTER_HANDLE handle, UINT64 address, void* buffer, size_t size);

	template <typename T>
	void AddScatterReadRequest(VMMDLL_SCATTER_HANDLE handle, UINT64 address, T* buffer)
	{
		AddScatterReadRequest(handle, address, reinterpret_cast<void*>(buffer), sizeof(T));
	}
		
	void AddScatterWriteRequest(VMMDLL_SCATTER_HANDLE handle, UINT64 address, void* buffer, size_t size);
