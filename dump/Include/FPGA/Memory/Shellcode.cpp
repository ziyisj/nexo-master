...
UINT64 c_shellcode::find_codecave(size_t function_size, const std::string& process_name, const std::string& module)
{
...
std::vector<UINT64> c_shellcode::find_all_codecave(size_t function_size, const std::string& process_name)
{
	std::vector<UINT64> codecaves = { };
	...
	for (size_t i = 0; i < module_list.size(); i++)
	{
		...
		UINT64 codecave = find_codecave(function_size, process_name, module);
		if (!codecave)
			continue;
...
bool c_shellcode::call_function(void* hook, void* function, const std::string& process_name)
{
	...
	// Set jump address
	*reinterpret_cast<UINT64*>(jmp_bytes + 6) = reinterpret_cast<uintptr_t>(hook);
