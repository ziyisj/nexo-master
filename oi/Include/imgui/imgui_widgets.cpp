...
bool ImGui::Keybind(const char* label, int* key, bool show_label)
{
    ...
    // 为清除按钮预留的空间
    float clearButtonWidth = 0;
    if (*key != 0) {
        clearButtonWidth = rect.GetHeight() + 4.0f; // 按钮大小 + 间距
    }

    // 向左移动按键显示区域，为清除按钮腾出空间
    ImRect clickable(
        ImVec2(rect.Max.x - 8 - label_size.x - clearButtonWidth, rect.Min.y),
        ImVec2(rect.Max.x - clearButtonWidth, rect.Max.y)
    );

    // 清除按钮位置
    ImRect clearButtonRect;
    if (*key != 0) {
        clearButtonRect = ImRect(
            ImVec2(clickable.Max.x + 4.0f, clickable.Min.y),
            ImVec2(clickable.Max.x + 4.0f + rect.GetHeight(), clickable.Min.y + rect.GetHeight())
        );
    }
    ...
    // 检查清除按钮是否被悬浮和点击
    ImGuiID clearButtonId = window->GetID((label + std::string("_clear_button")).c_str());
    bool clearButtonHovered = (*key != 0) && ItemHoverable(clearButtonRect, clearButtonId, NULL);
    bool clearButtonClicked = clearButtonHovered && io.MouseClicked[0];
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, g.ActiveId == id ? Style::AccentTextColor : hovered ? Style::Text::TextHov : Style::Text::Text, ImGui::GetIO().DeltaTime * 6.f);

    window->DrawList->AddRectFilled(clickable.Min + ImVec2(-6, 0), clickable.Max - ImVec2(0, 0), GetColorU32(Style::Keybind::Background), Style::Keybind::Rounding);
    ...
    if (*key != 0) {
        ...
        window->DrawList->AddRectFilled(
            clearButtonRect.Min,
            clearButtonRect.Max,
            GetColorU32(clearButtonHovered ? IM_COL32(50, 50, 50, 255) : GetColorU32(Style::Keybind::Background)),
            Style::Keybind::Rounding
        );

        const char* x_symbol = U8("×");
        ImVec2 symbol_size = ImGui::CalcTextSize(x_symbol);

        // 计算按钮的中心点
        float centerX = (clearButtonRect.Min.x + clearButtonRect.Max.x) * 0.5f;
        float centerY = (clearButtonRect.Min.y + clearButtonRect.Max.y) * 0.5f;

        // 绘制"×"符号，确保居中
        window->DrawList->AddText(
            ImVec2(centerX - symbol_size.x * 0.5f + 0.999f, centerY - symbol_size.y * 0.5f - 1.0f),
            GetColorU32(IM_COL32(200, 200, 200, 255)),
            x_symbol
        );
    }
    ...
    PushStyleColor(ImGuiCol_Text, Style::AccentTextColor);
    RenderTextClipped(clickable.Min + ImVec2(-3, 0), clickable.Max, buf_display, NULL, &label_size, ImVec2(0.5f, 0.5f));
    PopStyleColor();

    // 处理清除按钮点击
    if (clearButtonClicked && *key != 0) {
        *key = 0;
        value_changed = true;
    }
    // 处理按键绑定区域的点击
...
bool ImGui::Tab(bool selected, const char* label, const ImVec2& size_arg)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ...
    ImVec2 size = ImGui::CalcItemSize(size_arg, NULL, NULL);
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, selected || hovered ? Style::AccentTextColor : hovered ? Style::Text::TextHov : Style::Text::Text, g.IO.DeltaTime * 12.f);

    if (selected || hovered) {
        window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(Style::Tab::Background), Style::Tab::Rounding);
        //ImGui::PushStyleColor(ImGuiCol_Text, GetColorU32(Style::WhiteTextColor));
    }
    ImGui::PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    ImGui::RenderTextClipped(bb.Min + ImVec2(13.f, label_size.y / 2), bb.Max, label, NULL, &label_size, ImVec2(0.0f, 0.0f), &bb);
    ImGui::PopStyleColor();
...
bool ImGui::SubTab(bool selected, const char* label, ImVec2 size_arg, bool useBase, float w)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ...
    static std::map<ImGuiID, SubTabState> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, SubTabState() });
        it_anim = anim.find(id);
    }

    it_anim->second.text = ImLerp(it_anim->second.text, selected || hovered ? Style::SubTab::AccentTextColor : hovered ? Style::SubTab::AccentTextColor : Style::Text::Text, g.IO.DeltaTime * 12.f);

    if (selected || hovered)
    {
        window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(Style::SubTab::Background), Style::SubTab::Rounding);
    }
    else {
        window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(useBase ? Style::SubTab::BaseBackground : Style::SubTab::DefBackground), Style::SubTab::Rounding);
    }

    ImGui::PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    //ImGui::RenderTextClipped(bb.Min + ImVec2(13.f, label_size.y / 4), bb.Max, label, NULL, &label_size, ImVec2(0.0f, 0.0f), &bb);
...
bool ImGui::NewCheckbox(const char* label, bool* v)
{
    ...
    const float square_sz = 16.f, w = GetContentRegionMax().x - style.WindowPadding.x - square_sz;
    const ImVec2 pos = window->DC.CursorPos;
    ...
    // 使用circle成员作为背景色动画
    // printf("Style::AccentColor: %f, %f, %f\n", 		GameData.Config.Theme.Active[0],
	// 	GameData.Config.Theme.Active[1],
	// 	GameData.Config.Theme.Active[2]);

    it_anim->second.circle = ImLerp(it_anim->second.circle, *v ? Style::AccentColor : Style::Checkbox::Background, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, *v ? Style::Text::Text : hovered ? Style::Text::TextHov : Style::Text::Text, g.IO.DeltaTime * 6.f);
    it_anim->second.move = ImLerp(it_anim->second.move, *v ? 5.f : -12.f, g.IO.DeltaTime * 6.f);

    // 计算对号透明度
    float checkmark_alpha = ImClamp((it_anim->second.move + 12.f) / 17.f, 0.0f, 1.0f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));
    const ImVec2 checkbox_pos = ImVec2(check_bb.Min.x + w - 1, check_bb.Min.y);
    const ImVec2 checkbox_size = ImVec2(square_sz, square_sz);

    // 绘制复选框背景
    GetWindowDrawList()->AddRectFilled(
        checkbox_pos,
        checkbox_pos + checkbox_size,
        GetColorU32(it_anim->second.circle),
        Style::Checkbox::Rounding
    );
    ...
    GetWindowDrawList()->AddRect(
        checkbox_pos,
        checkbox_pos + checkbox_size,
        GetColorU32(Style::Checkbox::OutlineBackground),
        Style::Checkbox::Rounding
    );

    // 使用ImGui内置的RenderCheckMark函数绘制对号
    if (*v) {
        // 选中状态：使用白色对号
        ImVec4 checkmark_color = Style::BlackTextColor;
        RenderCheckMark(
            GetWindowDrawList(),
            checkbox_pos + ImVec2(square_sz * 0.25f, square_sz * 0.25f),
            GetColorU32(checkmark_color),
            square_sz * 0.5f
        );
    }
    else {
        //// 未选中状态：使用灰色对号
        //ImVec4 gray_checkmark_color = ImVec4(0.6f, 0.6f, 0.6f, 0.4f);
        //RenderCheckMark(
        //    GetWindowDrawList(),
        //    checkbox_pos + ImVec2(square_sz * 0.25f, square_sz * 0.25f),
        //    GetColorU32(gray_checkmark_color),
        //    square_sz * 0.5f
        //);
    }
...
bool ImGui::NewCheckboxType(const char* label, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const float square_sz = 16.f, w = 0;
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(w + square_sz, label_size.y + 5));
    ItemSize(total_bb, 0.f);
    ...
    it_anim->second.circle = ImLerp(it_anim->second.circle, *v ? Style::AccentColor : Style::Checkbox::CircleInactiveType, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, *v ? Style::AccentTextColor : hovered ? Style::Text::TextHov : Style::Text::Text, g.IO.DeltaTime * 6.f);
    it_anim->second.move = ImLerp(it_anim->second.move, *v ? 9.f : -12.f, g.IO.DeltaTime * 6.f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));

    GetWindowDrawList()->AddRectFilled(check_bb.Min + ImVec2(w - 1, 0), check_bb.Max + ImVec2(w, 0), GetColorU32(Style::Checkbox::BackgroundType), 30.f);


    GetWindowDrawList()->AddCircleFilled(ImVec2(check_bb.Min.x + w + square_sz / 2.11f, check_bb.Max.y - square_sz / 2.f), 5.f, GetColorU32(it_anim->second.circle), 100.f);
    GetWindowDrawList()->AddShadowCircle(ImVec2(check_bb.Min.x + w + square_sz / 2.11f, check_bb.Max.y - square_sz / 2.f), 5.f, GetColorU32(it_anim->second.circle), 50.f, ImVec2(0, 0), 0, 12);
...
bool ImGui::NewCheckboxOnLeft(const char* label, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const float spacing = 6.f; // 复选框和文字的间距
    const ImVec2 pos = window->DC.CursorPos;

    // 复选框和文字并排，整体区域
    const ImRect total_bb(pos, pos + ImVec2(square_sz + spacing + label_size.x, ImMax(square_sz, label_size.y) + 5));
    ItemSize(total_bb, 0.f);
    ...
    // 动画
    it_anim->second.circle = ImLerp(it_anim->second.circle, *v ? Style::AccentColor : Style::Checkbox::BaseBackground, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, *v ? ImGui::ColorConvertU32ToFloat4(ImColor(245, 245, 230)) : hovered ? Style::Text::TextHov : Style::Text::Text, g.IO.DeltaTime * 6.f);
    it_anim->second.move = ImLerp(it_anim->second.move, *v ? 5.f : -12.f, g.IO.DeltaTime * 6.f);
    ...
    // 绘制标签文本（紧贴复选框右侧）
    ImVec2 text_pos = ImVec2(checkbox_pos.x + square_sz + spacing, checkbox_pos.y + (square_sz - label_size.y) * 0.5f - 1);
    GetWindowDrawList()->AddText(text_pos, GetColorU32(it_anim->second.text), label);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    //const ImU32 col = ColorConvertFloat4ToU32((held && hovered) ? ImVec4(GameData.Config.Theme.Active[0], GameData.Config.Theme.Active[1], GameData.Config.Theme.Active[2], GameData.Config.Theme.Active[3]) : hovered ? ImVec4(GameData.Config.Theme.Active[0], GameData.Config.Theme.Active[1], GameData.Config.Theme.Active[2], 0.8f) : ImVec4(GameData.Config.Theme.Active[0], GameData.Config.Theme.Active[1], GameData.Config.Theme.Active[2], GameData.Config.Theme.Active[3]));
...
bool ImGui::NewBeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, bool hide_lable)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    const ImRect bb(window->DC.CursorPos + (hide_lable ? ImVec2(0, -2) : ImVec2(100, -2)), window->DC.CursorPos + (hide_lable ? ImVec2(100.f, label_size.y + 2) : ImVec2(w, label_size.y + 2)));
    ...
    static std::map<ImGuiID, ComboState> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, ComboState() });
        it_anim = anim.find(id);
    }

    it_anim->second.text = ImLerp(it_anim->second.text, popup_open ? Style::AccentTextColor : hovered ? Style::Text::TextHov : Style::Text::Text, g.IO.DeltaTime * 16.f);
    it_anim->second.combo = ImLerp(it_anim->second.combo, popup_open ? Style::AccentColor : hovered ? Style::AccentLowColor : Style::Text::Text, g.IO.DeltaTime * 16.f);

    const float value_x2 = ImMax(bb.Min.x, bb.Max.x - arrow_size);

    GetWindowDrawList()->AddRectFilled(bb.Min, ImVec2(value_x2, bb.Max.y), GetColorU32(Style::Combo::Background), Style::Combo::Rounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersLeft);

    GetWindowDrawList()->AddRectFilled(ImVec2(value_x2, bb.Min.y), bb.Max, GetColorU32(Style::Combo::Background), Style::Combo::Rounding, (w <= arrow_size) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersRight);

    GetWindowDrawList()->AddRect(bb.Min, ImVec2(bb.Min.x + (w - 100), bb.Max.y), GetColorU32(Style::Combo::OutlineBackground), Style::Combo::Rounding);

    PushStyleColor(ImGuiCol_Text, GetColorU32(Style::AccentTextLowColor));
    ...
    if (!hide_lable) GetWindowDrawList()->AddText(ImVec2(bb.Max.x - w, bb.Min.y + 2), GetColorU32(it_anim->second.text), label);

    if (!popup_open) return false;

    g.NextWindowData.Flags = backup_next_window_data_flags;
    return NewBeginComboPopup(popup_id, bb, flags);
} 

bool ImGui::NewBeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ImGuiContext& g = *GImGui;
    if (!IsPopupOpen(popup_id, ImGuiPopupFlags_None))
    {
    ...
    PushStyleVar(ImGuiStyleVar_PopupRounding, Style::Combo::Rounding);

    PushStyleColor(ImGuiCol_PopupBg, GetColorU32(Style::Combo::Background));
    PushStyleColor(ImGuiCol_Border, GetColorU32(Style::Combo::OutlineBackground));
    bool ret = Begin(name, NULL, window_flags);
    PopStyleColor(2);
...
// Getter for the old Combo() API: const char*[]
static bool New_Items_ArrayGetter(void* data, int idx, const char** out_text)
{
    const char* const* items = (const char* const*)data;
    if (out_text)
        *out_text = items[idx];
...
// Old API, prefer using BeginCombo() nowadays if you can.
bool ImGui::NewCombo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items, bool hide_lable)
{
    ImGuiContext& g = *GImGui;
    ...
    if (!NewBeginCombo(label, preview_value, ImGuiComboFlags_None, hide_lable))
        return false;
...
// Combo box helper allowing to pass an array of strings.
bool ImGui::NewCombo(const char* label, int* current_item, const char* const items[], int items_count, int height_in_items, bool hide_lable)
{
    const bool value_changed = NewCombo(label, current_item, New_Items_ArrayGetter, (void*)items, items_count, height_in_items, hide_lable);
    return value_changed;
}

// Combo box helper allowing to pass all items in a single string literal holding multiple zero-terminated items "item1\0item2\0"
bool ImGui::NewCombo(const char* label, int* current_item, const char* items_separated_by_zeros, int height_in_items, bool hide_lable)
{
    int items_count = 0;
    const char* p = items_separated_by_zeros;       // FIXME-OPT: Avoid computing this, or at least only when combo is open
    while (*p)
...
void ImGui::NewMultiCombo(const char* label, bool variable[], const char* labels[], int count)
{
    ImGuiContext& g = *GImGui;
    ...
    if (ImGui::NewBeginCombo(label, preview.c_str(), count)) // draw start
    {
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, const char* unit, bool useBase)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y));
    ImRect slider_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y));
    ...
    ItemSize(ImRect(total_bb.Min, total_bb.Max + ImVec2(0, 4 + 20)));
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? Style::AccentTextColor : hovered ? Style::Text::TextHov : Style::Text::Text, g.IO.DeltaTime * 6.f);
    
    GetWindowDrawList()->AddRectFilled(slider_bb.Min + ImVec2(0, 4), slider_bb.Max - ImVec2(0, 4), GetColorU32(useBase ? Style::Child::Background : Style::Slider::Background), 30.f);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        it_anim->second.slow = ImLerp(it_anim->second.slow, grab_bb.Min.x - frame_bb.Min.x, g.IO.DeltaTime * 15.f);

        GetWindowDrawList()->AddRectFilled(slider_bb.Min + ImVec2(0, 4), ImVec2(it_anim->second.slow + frame_bb.Min.x + 5, grab_bb.Max.y - 4), GetColorU32(Style::AccentColor), 30.f);
        GetWindowDrawList()->AddCircleFilled(ImVec2(frame_bb.Min.x + it_anim->second.slow + 5, grab_bb.Max.y - 6.5), 6.f, GetColorU32(Style::Slider::CircleInactive), 30.f);
        //GetWindowDrawList()->AddCircleFilled(ImVec2(frame_bb.Min.x + it_anim->second.slow + 5, grab_bb.Max.y - 6.5), 6.f, GetColorU32(Style::BlackTextColor), 30.f);
        //float width = 8.f;  // 矩形宽度
        //float height = 8.f; // 矩形高度
        //ImVec2 center(frame_bb.Min.x + it_anim->second.slow + 5, grab_bb.Max.y - 6.5); // 原圆心位置作为中心

        //GetWindowDrawList()->AddRectFilled(
        //    ImVec2(center.x - width / 2, center.y - height / 2),
        //    ImVec2(center.x + width / 2, center.y + height / 2),
        //    GetColorU32(Style::Slider::CircleInactive),
        //    1.f);
    }
   ...
   // GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(w - 40, 0), frame_bb.Max + ImVec2(0, 1), GetColorU32(Style::Slider::Background), Style::Slider::Rounding);
    //GetWindowDrawList()->AddRect(frame_bb.Min + ImVec2(w - 40, 0), frame_bb.Max + ImVec2(0, 1), GetColorU32(c::slider::outline_background), Style::Slider::Rounding);

    //PushFont(font::poppins_medium_low);
    //PushStyleColor(ImGuiCol_Text, GetColorU32(Style::AccentTextColor));
    //RenderTextClipped(frame_bb.Max - ImVec2(50, 20), frame_bb.Max + ImVec2(50, 4), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
 // 创建带单位的字符串
    std::string text_with_unit = std::string(value_buf) + " " + unit;

    // 计算文本宽度 - 注意第二个参数是NULL，因为c_str()已经是null终止字符串
    ImVec2 text_size = ImGui::CalcTextSize(text_with_unit.c_str(), NULL);

    // 计算靠右位置，保持固定的右边距
    ImVec2 text_pos = frame_bb.Max - ImVec2(0, 20); // 10是右边距
    text_pos.x -= text_size.x; // 根据文本宽度向左偏移

    // 渲染文本 - 同样，第四个参数应该是NULL
    ImGui::PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    ImGui::RenderTextClipped(
        text_pos,
        frame_bb.Max - ImVec2(0, -4), // 保持固定的右边距
        text_with_unit.c_str(),
        NULL,  // 这里应该是NULL，不是value_buf_end
        NULL,
        ImVec2(0.0f, 0.5f) // x=0表示左对齐，我们已经手动计算了位置
    );
    ImGui::PopStyleColor();
    //PopStyleColor();
    //PopFont();
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags, const char* unit)
{
    return SliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, format, flags, unit);
}
...
bool ImGui::SliderInt(const char* label, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags, const char* unit, bool useBase)
{
    return SliderScalar(label, ImGuiDataType_S32, v, &v_min, &v_max, format, flags, unit, useBase);
}
...
bool ImGui::NewInputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
    return NewInputTextEx(label, NULL, buf, (int)buf_size, ImVec2(0, 0), flags, callback, user_data);
}
...
bool ImGui::NewInputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (!is_multiline)
    {
        GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(Style::Input::Background), Style::Input::Rounding);
        GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(Style::Input::OutlineBackground), Style::Input::Rounding);
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ImU32 col = GetColorU32(is_displaying_hint ? Style::Text::Text : Style::AccentLow);
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(total_bb.Min + ImVec2(it_anim->second.slow, 3), total_bb.GetBL() - ImVec2(-it_anim->second.slow, 0), GetColorU32(Style::Text::Text));
...
bool ImGui::NewColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        const ImVec4 col_v4(col[0], col[1], col[2], alpha ? col[3] : 1.0f);
        if (NewColorButton("##ColorButton", col_v4, flags))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                // Store current color and open a picker
        ...
        PushStyleColor(ImGuiCol_PopupBg, GetColorU32(Style::Picker::Background));
        PushStyleColor(ImGuiCol_Border, GetColorU32(Style::Picker::OutlineBackground));

        PushStyleVar(ImGuiStyleVar_PopupRounding, Style::Picker::Rounding);
        ...
        if (BeginPopup("picker"))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                ImGuiColorEditFlags picker_flags = (flags_untouched & picker_flags_to_forward) | ImGuiColorEditFlags_DisplayMask_ | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_AlphaPreviewHalf;
                SetNextItemWidth(square_sz * 12.0f); // Use 256 + bar sizes?
                value_changed |= NewColorPicker4("##picker", col, picker_flags, &g.ColorPickerRef.x);
            }
            EndPopup();
        }
        PopStyleColor(2);
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        ImGui::GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(0, 1), GetColorU32(Style::Text::Text), label);
...
{
    SetCursorPos(GetCursorPos() + ImVec2(GetContentRegionMax().x - 290, 0));
    ImGui::NewColorEdit4(label_picker, col, ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_NoLabel);
    SetCursorPos(GetCursorPos() - ImVec2(0, 31));
    ImGui::NewCheckbox(label, v);
}
...
bool ImGui::NewColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    // Render SV Square
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), col_white, hue_color32, hue_color32, col_white, Style::Picker::Rounding, ImDrawCornerFlags_All);
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), 0, 0, col_black, col_black, Style::Picker::Rounding, ImDrawCornerFlags_All);
    RenderFrameBorder(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), 0.0f);
    sv_cursor_pos.x = ImClamp(IM_ROUND(picker_pos.x + ImSaturate(S) * sv_picker_size), picker_pos.x + 2, picker_pos.x + sv_picker_size - 2); // Sneakily prevent the circle to stick out too much
    ...
    if (alpha_bar)
    {
        ...
        GetForegroundDrawList()->AddRectFilledMultiColorRounded(bar1_bb.Min + ImVec2(0, 8), bar1_bb.Max - ImVec2(0, 5), GetColorU32(ImGuiCol_PopupBg), 0, 0, 0, 0, Style::Picker::Rounding, ImDrawCornerFlags_All);
...
bool ImGui::NewColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderColorRectWithAlphaCheckerboard(window->DrawList, clickable.Min, clickable.Max + ImVec2(0, 0), GetColorU32(col_rgb), Style::Picker::Rounding, ImVec2(0.f, 0.f), Style::Picker::Rounding);
    GetWindowDrawList()->AddShadowRect(clickable.Min, clickable.Max + ImVec2(0, 0), GetColorU32(col_rgb), 20, ImVec2(0.f, 0.f), Style::Picker::Rounding, 16);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? ImVec4(1, 1, 1, 1) : hovered ? Style::Text::TextHov : Style::Text::Text, g.IO.DeltaTime * 22.f);
