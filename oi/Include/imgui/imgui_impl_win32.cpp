...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CHAR:
        ...
        {
            // 对于多字节字符集，检查是否是双字节字符
            if (wParam >= 0x81 && wParam <= 0xFE) // 中文字符范围
            {
                // 这是双字节字符，但我们只能获得一个字节
                // 我们需要等待下一个 WM_CHAR 消息
                static unsigned char firstByte = 0;
                if (firstByte == 0)
                {
                    firstByte = (unsigned char)wParam;
                }
                else
                {
                    // 组合两个字节
                    char mbStr[3] = { (char)firstByte, (char)wParam, 0 };
                    wchar_t wch = 0;
                    ::MultiByteToWideChar(CP_ACP, 0, mbStr, 2, &wch, 1);
