...
public:
	...
	* @return true if successful, false if not.
	*/
	bool Init(bool memMap = true, bool debug = false, bool norefresh = true);
	...
	*/
	...
	size_t GetBaseDaddy(std::string module_name, int pid);
	...
	*/
	...
	std::vector<uint64_t> FindSignatureAll(const char* signature, uint64_t range_start, uint64_t range_end, int PID = 0);
	...
	* @return true if successful, false if not.
	*/
	bool Read(uintptr_t address, void* buffer, DWORD size) const;
	bool ReadNotCache(uintptr_t address, void* buffer, DWORD size) const;
	bool Read(uintptr_t address, void* buffer, DWORD size, int pid) const;
	...
	*/
	...
	template <typename T>
	T ReadNotCache(void* address)
	{
		T buffer{ };
		memset(&buffer, 0, sizeof(T));
		ReadNotCache(reinterpret_cast<uint64_t>(address), reinterpret_cast<void*>(&buffer), sizeof(T));
	...
	template <typename T>
	T ReadNotCache(uint64_t address)
	{
		return ReadNotCache<T>(reinterpret_cast<void*>(address));
	}
	...
	*/
	...
	VMMDLL_SCATTER_HANDLE CreateScatterHandleUseCache();
	VMMDLL_SCATTER_HANDLE CreateScatterHandleTwo();
	...
	template <typename T>
	void AddScatterRead(VMMDLL_SCATTER_HANDLE handle, uint64_t address, T* buffer) {
		AddScatterReadRequest(handle, address, static_cast<void*>(buffer), sizeof(T));
	}

	template<typename T>
	void AddScatterWrite(VMMDLL_SCATTER_HANDLE handle, uint64_t address, const T& buffer) {
		AddScatterWriteRequest(handle, address, (void*)&buffer, sizeof(T));
	}
	...
	void ExecuteReadScatterUseCache(VMMDLL_SCATTER_HANDLE handle, int pid = 0);
	void ExecuteWriteScatter(VMMDLL_SCATTER_HANDLE handle, int pid = 0);

	template<typename T>
	std::vector<T> ReadVector(uintptr_t address, size_t size, size_t custom_type_size = NULL)
	{
		std::vector<T> temp{};
		...
		if (Read(address, &temp[0], custom_type_size ? custom_type_size : sizeof(T) * size))
			return temp;

		return temp;
	}

	uint64_t GetModuleBase(std::string ModuleName)
	{
		return VMMDLL_ProcessGetModuleBaseU(this->vHandle, this->current_process.PID, ModuleName.c_str());
	}

	uint64_t GetModuleBase(std::string ModuleName, UINT32 PID)
	{
		return VMMDLL_ProcessGetModuleBaseU(this->vHandle, PID, ModuleName.c_str());
	}
