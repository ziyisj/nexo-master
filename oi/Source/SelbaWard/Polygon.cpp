...
void Polygon::priv_triangulateEarClip()
{
	...
	// UEngineh index is treated as a separate vertex, even if it's technically the same vertex as another (same vertex numbers)
	...
	if (!m_holeStartIndices.empty())
	{
		...
		for (std::size_t h{ 0u }; h < holeInfo.size(); ++h)
		{
			...
			// indices for UEngineh of the vertex numbers (allowing re-using of vertices - needed for cutting polygon)
...
void Polygon::priv_triangulateBasicEarClip()
{
	...
	// indices for UEngineh of the vertices/points
