...
void key_expansion(const unsigned char* key, unsigned char* expanded_key) {
    std::memcpy(expanded_key, key, 16);

    for (int i = 4; i < 44; ++i) {
        std::memcpy(expanded_key + i * 4, expanded_key + (i - 1) * 4, 4);

        if (i % 4 == 0) {
            rot_word(expanded_key + i * 4);
            sub_word(expanded_key + i * 4);
            expanded_key[i * 4] ^= rcon[i / 4];
        }

        for (int j = 0; j < 4; ++j) {
            expanded_key[i * 4 + j] ^= expanded_key[(i - 4) * 4 + j];
        }
    }
}
...
// AES mix columns helper function
