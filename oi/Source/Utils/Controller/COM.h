...
protected:
    ...
    inline void set_com_port(int port)
    {
        ...
        _port = port;
        strcpy(_com_str, "\\\\.\\COM");
        _ltoa(_port, p, 10);  // 使用 _ltoa 替换 ltoa
        strcat(_com_str, p);
    }
public:
    _base_com()
...
public:
    ...
    inline int writeHex(const char* hexString) {
        assert(hexString);
        ...
        unsigned long w_len = 0, o_len = 0;
        if (!WriteFile(_com_handle, buf, bufSize, &w_len, &_wo)) {
            if (GetLastError() != ERROR_IO_PENDING) {
                // Handle the error
                delete[] buf;
                return 0;
            ...
            if (!GetOverlappedResult(_com_handle, &_wo, &w_len, TRUE)) {
                // Handle the error
                delete[] buf;
                return 0;
            }
